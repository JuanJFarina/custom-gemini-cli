...
- **Current scheduled-agent scope**: Event notifications use a special scheduler entrypoint that shares profiles, prior conversation loading, current time and weather, Gemini, Google Search grounding, and the core reason-and-act loop with normal conversations. They currently receive no application tools and are not persisted into conversation history.
...
- **Exact subscription periods**: The external product should synchronize each user's exact current `subscription_period_starts_at` and `subscription_period_ends_at` UTC boundaries. Conversation and event-notification allowances should use those boundaries instead of deriving a cycle from account creation, an original subscription date, or UTC calendar months.
- **Separate event-notification quotas**: Each plan should have a per-subscription-period event-notification allowance separate from conversation quota. The provisional free, basic, and max limits are 15, 60, and 240 successful notifications.
- **Notification quota accounting**: Successful Telegram deliveries for both `user_event` and `system_event` occurrences should count once within the user's synchronized subscription period. Reservations should happen before Gemini, while failed deliveries, retries, interaction-event messages, and quota notices should consume no allowance.
- **Notification quota visibility**: Relevant event flows should expose the plan limit, remaining allowance, and synchronized subscription-period end. The first blocked occurrence in a subscription period should receive one static, non-Gemini quota-exhausted notice instead of failing silently.
...
- **Scheduled-message history**: A delivered `user_event`, `system_event`, or `interaction_event` message should be persisted as a standalone assistant message with no fabricated user prompt. Scheduled messages and their read-only tool interactions should be available to later conversation context without consuming conversation quota.
- **Scheduled read-only capabilities**: Scheduled agent runs should receive the user's profiles, conversation context, current time and weather, Google Search grounding, and all authorized read-only tools. Modifying tools should be absent from the runtime tool store rather than restricted only through instructions.
- **Interaction events**: Every user should have one internal `interaction_event` that can be disabled and re-enabled but not deleted. It has no fixed schedule, duration, recurrence, or notification quota and exists only to let the scheduler consider a proactive conversational interaction.
- **Interaction-event ordering**: After processing ordinary due events, each scheduler round should consider an active interaction event only for a user who had no due `user_event` or `system_event` in that round. One user's due event should not suppress another user's interaction event.
- **Interaction-event probability**: On each five-minute check, the interaction probability should grow from the most recent contact, defined as the later of the user's latest message and any successfully delivered assistant message. A delivered assistant response, event notification, or interaction message resets the probability clock.
- **Calm interaction cadence**: Interaction events should use a scheduler-interval-independent Weibull probability with shape `2` and scale `96 hours`. For elapsed time `t` and scheduler interval `Δ`, the next-round probability is `1 - exp(-(((t + Δ) / 96h)^2 - (t / 96h)^2))`, corresponding to cumulative probabilities of about 6% after one day, 22% after two days, 43% after three days, and 63% after four days without contact.
- **Interaction-event cutoff**: Harle should not send an interaction message after seven days without an actual user message. The event should remain active and automatically become eligible again after the user returns; it should not wait for or require a response to any earlier interaction message.
- **Interaction-event capabilities**: An interaction run should receive conversation and profile context, current time and weather, Google Search grounding, and all authorized read-only tools. Modifying tools must be absent at runtime. Interaction events should have no quiet period and should consume neither conversation nor event-notification quota.
