...
- **Event types and notification state**: Events are `user_event` agenda records or `system_event` assistant tasks, preserve an IANA timezone and UTC boundaries, and store a notification window and delivery state.
- **Event notifications**: Events default to a notification window 15 minutes before their start. A process-local `AgentsScheduler` checks every five minutes, wakes the owning active user's agent without modifying tools, sends a concise Telegram notification, and marks the event notified only after successful delivery.
- **Event notification policy**: Rescheduling preserves the configured lead unless the user changes it, changing the start or notification window resets delivery state, failed delivery retries while the event remains in the future, and scheduled notifications do not consume conversation quota.
