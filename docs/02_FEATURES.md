...
- **Event notification policy**: The scheduler compares `last_notified_at` with each computed notification window, retries failed delivery while the occurrence remains in the future, skips missed occurrences, and does not consume conversation quota.
